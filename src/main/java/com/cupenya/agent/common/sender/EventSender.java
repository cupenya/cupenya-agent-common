...
public class EventSender {
  ...
  private final CommonAgentConfig agentConfig;

  private final Queue<DefaultEvent> queue;

  private final Thread sender;

  public EventSender(CommonAgentConfig agentConfig) throws IOException {
  ...
  private Queue<DefaultEvent> createQueue() throws IOException {
    if (agentConfig.type == CommonAgentConfig.QueueType.file) {
      log.info("Init with file backed queue");
      return new MemoryFileBackedQueue<DefaultEvent>(new File(agentConfig.file),
        agentConfig.memorySize);
    } else {
  ...
  public void queueEvent(DefaultEvent event) throws IOException {
    if (queue.size() < agentConfig.maxSize) {
      queue.offer(event);
    } else {
      log.warn(
        "Dropping event " + event.getId() + " because queue is at max size of " + agentConfig.maxSize);
  ...
  private void sendEvent(DefaultEvent event) throws IOException {
    ...
    wrapper.setApiKey(agentConfig.apiKey);
  ...
  private void sendEvent0(String eventContentJson, String apiPath)
    ...
    HttpPost post = new HttpPost(agentConfig.endpoint + apiPath);
